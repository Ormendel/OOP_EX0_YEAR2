...
public class Monom implements function 
{
	...
	// ***************** add your code below **********************
	/**
	 * The main method in this program to extract the monom out of 's' is by matches function - stated in String object.
	 * @param s represents the Monom
	 */
	public Monom(String s) 
	{
		...
		else if(s.matches("([+-]?[0-9]*\\.?[0-9]*\\*?[xX](\\^[0-9]+)?)|([+-]?[0-9]*\\.?[0-9]*?)"))
		{
				...
				// Real number [not(^) and not(x)]
			{
				...
				{
					throw new RuntimeException("The coefficient"+Double.parseDouble(s)+" is unvalid and supposed to be a number");
				}
				this.set_power(0);	
			}
	...
	public String toString()
	{
		...
		String ans = this._coefficient + "x" + "^" + this._power;//From the form ax^b
		return ans;
	}
	...
	{
		if (p < 0) 
			throw new RuntimeException("Error; The power of Monom should not be negative, got: " + p);
		else
			this._power = p;
	}
